...
def get_data(filters):
    ...
    data =  frappe.db.sql(
        f"""
        SELECT 
            count(si.name) as number_of_due_monthes, si.vehicle as license_plate_no,iv.status, iv.monthly_share_amount as share_amount, iv.enable_gps as gps_subscription , iv.enable_garage, iv.vehicle_code,  si.customer, SUM(si.grand_total) as total_due
        FROM `tabSales Invoice` si
        JOIN   `tabInstallment Vehicle`  iv on iv.name = si.custom_installment_vehicle
        WHERE {conditions}
        and si.status = 'Overdue'
        and si.remarks = '{invoice_setting.share}'
        GROUP BY si.vehicle, si.customer
        """,
        filters,
        as_dict=True
    )
